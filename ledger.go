...
func (c *WalmartClient) GetOrderLedger(ctx context.Context, orderID string) (*OrderLedger, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("marshaling request variables: %w", err)
	}
	params := url.Values{}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if attempt > 0 {
			...
			// Here attempt is > 0, so the shift count is non-negative.
		...
		ledger, retry, err := c.fetchLedgerAttempt(ctx, endpoint, orderID, attempt, maxRetries)
		if err != nil {
			if retry {
				lastErr = err
				continue
...
// fetchLedgerAttempt performs a single ledger request attempt. It returns the
// parsed ledger on success. The retry return value is true when the caller
// should retry after backoff (the server responded with HTTP 429).
func (c *WalmartClient) fetchLedgerAttempt(ctx context.Context, endpoint, orderID string, attempt, maxRetries int) (ledger *OrderLedger, retry bool, err error) {
	...
	if resp.StatusCode == 429 {
		c.logger.Warn("rate limited, will retry",
			...
			slog.Int("attempt", attempt+1),
			slog.Int("max_attempts", maxRetries+1))
		return nil, true, fmt.Errorf("rate limited (attempt %d/%d)", attempt+1, maxRetries+1)
	}
	...
	if resp.StatusCode != http.StatusOK {
		if resp.StatusCode == 403 || resp.StatusCode == 418 {
			c.logger.Warn("access denied",
				slog.String("order_id", orderID),
				slog.Int("status_code", resp.StatusCode))
			return nil, false, fmt.Errorf("access denied (cookies might be stale) - try refreshing from browser")
		}
	...
	result := convertToOrderLedger(orderID, ledgerResp)

	c.logger.Info("fetched order ledger",
		slog.String("order_id", orderID),
		slog.Int("payment_method_count", len(result.PaymentMethods)),
		slog.Int("attempt", attempt+1))
